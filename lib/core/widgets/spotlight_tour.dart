...
class SpotlightTourPrefs {
  const SpotlightTourPrefs(this.storageKey);
...
class SpotlightTourStep {
  const SpotlightTourStep({
...
class SpotlightTourOverlay extends StatefulWidget {
  ...
  final List<SpotlightTourStep> steps;
...
class _SpotlightTourOverlayState extends State<SpotlightTourOverlay> {
  ...
  bool _skipOnLeft(Rect? hole) {
    ...
    return hole.width < 180 && hole.right > size.width - 88 && hole.top < 72;
  }
}
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(
      builder: (context, c) {
        ...
        const cardH = 220.0;
        final maxW = math.min(
          _SpotlightTourOverlayState._cardMaxWidth,
          c.maxWidth - 24,
        );
...
class _SpotlightPainter extends CustomPainter {
  ...
  bool shouldRepaint(covariant _SpotlightPainter old) =>
      old.hole != hole || old.color != color || old.holeRadius != holeRadius;
}
