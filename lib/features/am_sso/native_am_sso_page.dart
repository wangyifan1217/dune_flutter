...
import 'am_sso_service.dart';

/// 业务平台免登：服务端换票后在系统浏览器打开返回的 HTTPS 地址。
class NativeAmSsoPage extends StatefulWidget {
  const NativeAmSsoPage({
    super.key,
  ...
  @override
  State<NativeAmSsoPage> createState() => _NativeAmSsoPageState();
}

class _NativeAmSsoPageState extends State<NativeAmSsoPage> {
  late final AmSsoService _service = AmSsoService(widget.session);
  ...
  Future<void> _openAm() async {
    ...
    try {
      final login = await _service.fetchLoginUrl(widget.appKey);
      if (!mounted) return;
  ...
  String _friendlyError(Object error) {
    ...
    return text.isEmpty ? '${widget.title}暂时无法打开，请稍后重试' : text;
  }
        ...
        child: Container(
          ...
          child: Row(
            children: [
              ...
              Expanded(
                child: Text(
                  widget.title.isEmpty ? '免登' : widget.title,
