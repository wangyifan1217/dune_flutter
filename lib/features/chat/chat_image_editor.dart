...
/// IM 图片编辑（涂鸦 / 文字 / 裁剪 / 马赛克），交互接近微信。
abstract final class ChatImageEditor {
  /// [doneLabel]：发图前用「发送」，预览裁剪用「完成」。
  static ProImageEditorConfigs configs({String doneLabel = '发送'}) {
...
Future<Uint8List?> openChatImageEditor(
  ...
  return Navigator.of(context).push<Uint8List>(
    MaterialPageRoute(
      ...
      builder: (ctx) => ChatImageEditorPage(
        bytes: prepared,
        doneLabel: doneLabel,
      ),
    ),
...
class _ChatImageEditorPageState extends State<ChatImageEditorPage> {
  ...
  Widget build(BuildContext context) {
    return ProImageEditor.memory(
      widget.bytes,
      configs: ChatImageEditor.configs(doneLabel: widget.doneLabel),
