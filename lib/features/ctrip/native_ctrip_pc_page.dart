...
import 'ctrip_pc_service.dart';

/// 桌面端携程商旅 PC 单点入口。
///
/// 按官方要求：不嵌 iframe/WebView，取一次性 Form 后在系统浏览器打开。
class NativeCtripPCPage extends StatefulWidget {
  const NativeCtripPCPage({
    super.key,
  ...
  @override
  State<NativeCtripPCPage> createState() => _NativeCtripPCPageState();
}

class _NativeCtripPCPageState extends State<NativeCtripPCPage> {
  late final CtripPCService _service = CtripPCService(widget.session);
  ...
  Future<void> _openCtrip() async {
    ...
    try {
      ...
      if (!mounted) return;
      final opened = await _service.openInSystemBrowser(form);
      if (!mounted) return;
      if (!opened) {
        setState(() {
          _loading = false;
  ...
  Widget _buildBody() {
    ...
    if (_opened) {
      return _buildMessage(
        ...
        message: '已在系统浏览器中打开携程商旅 PC 版。\n预订完成后可关闭浏览器标签，返回沙丘继续工作。',
        actionLabel: '再次打开',
      );
    }
