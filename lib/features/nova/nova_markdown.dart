...
class NovaMarkdownBody extends StatelessWidget {
    ...
    if (raw.contains('```')) {
      for (final part in splitNovaMarkdownFences(raw)) {
        if (part.isCode) {
          ...
          widgets.addAll(
            _renderInlineSegment(context, part.body, shownNames, shownUrls),
          );
        }
      }
    } else {
  ...
  List<Widget> _renderInlineSegment(
    ...
    if (file != null && file.url.isNotEmpty) {
      ...
      if (rest.isNotEmpty)
        widgets.addAll(
          _renderInlineSegment(context, rest, shownNames, shownUrls),
        );
      widgets.add(
        _deliverableCard(context, file.copyWithExt(), shownNames, shownUrls),
      );
      return widgets;
    }
    ...
    if (bareRe.hasMatch(raw)) {
      ...
      for (final m in bareRe.allMatches(raw)) {
        ...
        widgets.add(
          _imageCard(
            context,
            NovaDeliverableItem(url: u, name: nm, ext: novaFileExt(u)),
            shownNames,
            shownUrls,
          ),
        );
    ...
    final withFiles = _renderTextWithGeneratedFiles(
      context,
      raw,
      shownNames,
      shownUrls,
  ...
  List<Widget> _renderTextWithGeneratedFiles(
    ...
    if (raw.trim().isEmpty || extractNovaMarkdownFileLinks(raw).isNotEmpty)
      return const [];
  ...
  List<Widget> _renderPlainOrBlock(
    ...
    return [
      _NovaMarkdownInline(
        ...
        documentPreview: documentPreview,
        onImage: (item) => _imageCard(context, item, shownNames, shownUrls),
        onFileLink: (label, url) =>
            _markdownLinkCard(context, label, url, shownNames, shownUrls),
      ),
    ];
  }
  ...
  Widget _markdownLinkCard(
    ...
    final item = NovaDeliverableItem(
      url: url,
      name: label,
      ext: novaFileExt(label).isNotEmpty
          ? novaFileExt(label)
          : novaFileExt(url.split('?').first),
    );
    if (novaIsImageFile(item))
      return _imageCard(context, item, shownNames, shownUrls);
  ...
  Widget _deliverableCard(
    ...
    if (novaIsImageFile(file))
      return _imageCard(context, file, shownNames, shownUrls);
  ...
  Widget _fileCard(
    ...
    return Padding(
      padding: const EdgeInsets.only(top: 8),
      child: NovaC4DeliverableFileCard(resolver: mediaResolver!, file: file),
    );
  }
}
...
class _NovaMarkdownInline extends StatelessWidget {
  ...
  Widget _buildWithImageWidgets(BuildContext context) {
    ...
    for (final m in imgRe.allMatches(text)) {
      ...
      if (nm.isEmpty) {
        try {
          nm = Uri.decodeComponent(
            m.group(2)!.split('/').last.split('?').first,
          );
        } catch (_) {
          nm = '图片';
        }
  ...
  void _parseInline(String input, List<InlineSpan> out, BuildContext context) {
    ...
    for (final m in re.allMatches(input)) {
      ...
      if (token.startsWith('**') && token.endsWith('**')) {
        ...
        final url = m.group(3)!;
        if (onFileLink != null &&
            (novaShouldRenderLinkAsFileCard(label, url) ||
                novaIsImageExt(novaFileExt(url)))) {
          // 由 block 层处理
